...
def prepare_querry()->dict:
    ...
    BASE_URL = os.environ["COVERITY_BASE_URL"] if is_env_set("COVERITY_BASE_URL") else None
    TOKEN =  os.environ["COVERITY_TOKEN"] if is_env_set("COVERITY_TOKEN") else None
    PROJECT_NAME =  os.environ["COVERITY_PROJECT_NAME"] if is_env_set("COVERITY_PROJECT_NAME") else None
    ...
    if None in [BASE_URL, TOKEN, PROJECT_NAME, USER]:
        ...
        log.error("Please set the following environment variables: COVERITY_BASE_URL, COVERITY_TOKEN, COVERITY_PROJECT_NAME, COVERITY_USER")
        return {}
    return {"base_url": BASE_URL,
             ...
             "user": USER, 
             "stream": PROJECT_NAME, 
             "columns": ["cid", "classification", "severity", "displayType", "fileLanguage",
