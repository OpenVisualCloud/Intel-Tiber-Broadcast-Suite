...
COV_CPP_JAVA_HOME="${COVERITY_CPP_BIN_DIR}/jre/bin/java"
COV_OTHER_JAVA_HOME="${COVERITY_OTHER_BIN_DIR}/jre/bin/java"

function export_cpp_java(){ 
  export JAVA_HOME="${COV_CPP_JAVA_HOME}"
}
function export_other_java(){ 
  export JAVA_HOME="${COV_OTHER_JAVA_HOME}"
}
function log_info(){ 
   echo "[INFO]: ${1}" 
}
