...
docker run -it \
  ...
  -udp_port 20000 -total_sessions 4 -ext_frames_mode 1 -f kahawai -i "0" \
  -framerate 50 -pixel_format y210le -width 3840 -height 2160 -port $NIC_PORT -local_addr $LOCAL_IP_ADDRESS -src_addr $SOURCE_IP_ADDRESS \
    -udp_port 20001 -total_sessions 4 -ext_frames_mode 1 -f kahawai -i "1" \
  -framerate 50 -pixel_format y210le -width 3840 -height 2160 -port $NIC_PORT -local_addr $LOCAL_IP_ADDRESS -src_addr $SOURCE_IP_ADDRESS \
    -udp_port 20002 -total_sessions 4 -ext_frames_mode 1 -f kahawai -i "2" \
  -framerate 50 -pixel_format y210le -width 3840 -height 2160 -port $NIC_PORT -local_addr $LOCAL_IP_ADDRESS -src_addr $SOURCE_IP_ADDRESS \
