...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // http://localhost:3209/settings/all -d "{\"logging_level\":-40}" # curl -X
    // PATCH -H "Content-Type: application/json"
    // http://localhost:3209/settings/all -T config.json

    if (argc > 1) {
      std::error_code error;
      node_model.settings =
          web::json::value::parse(utility::s2us(argv[1]), error);
      if (error) {
    ...
    if (!nmos::fields::error_log(node_model.settings).empty()) {
      error_log_buf.open(nmos::fields::error_log(node_model.settings),
    ...
    if (!nmos::fields::access_log(node_model.settings).empty()) {
      access_log_buf.open(nmos::fields::access_log(node_model.settings),
                          std::ios_base::out | std::ios_base::app);
      auto lock = log_model.write_lock();
    ...
    slog::log<slog::severities::info>(gate, SLOG_FLF)
        << "Initial settings: " << node_model.settings.serialize();

    // Set up the callbacks between the node server and the underlying
    // implementation

    // auto node_implementation = make_node_implementation(node_model,
    // config_manager, all_receivers, gate);
    auto node_implementation = make_node_implementation(
        node_model, config_manager, app_conn_resources, gate);
