...
// should be defined. it means that there are 3 senders of type video, 1 sender
// of type audio and 1 sender of type data
...
// frame_width, frame_height: control the frame_width and frame_height of video
// flows
const web::json::field_as_integer_or frame_width{U("frame_width"), 1920};
const web::json::field_as_integer_or frame_height{U("frame_height"), 1080};
...
=======
  ...
  const auto senders_count = impl::parse_count(impl::fields::senders_count(
      model.settings)); // max count of elements = 4 (because 4 types of ports:
                        // video, audio, mux, data)
  const auto senders_count_total =
      std::accumulate(senders_count.begin(), senders_count.end(), 0);
  // change
  const auto receivers_count = impl::parse_count(impl::fields::receivers_count(
      model.settings)); // max count of elements = 4 (because 4 types of ports:
                        // video, audio, mux, data)
...
make_node_implementation_system_global_handler(nmos::node_model &model,
                                               slog::base_gate &gate) {
...
make_node_implementation_registration_handler(slog::base_gate &gate) {
...
make_node_implementation_auto_resolver(const nmos::settings &settings,
  ...
  const auto device_id = impl::make_id(seed_id, nmos::types::device);
  const auto senders_count = impl::parse_count(impl::fields::senders_count(
      settings)); // max count of elements = 4 (because 4 types of ports: video,
                  // audio, mux, data)
  const auto senders_count_total =
      std::accumulate(senders_count.begin(), senders_count.end(), 0);
  const auto receivers_count = impl::parse_count(impl::fields::receivers_count(
  ...
  const auto ws_receiver_ports = boost::copy_range<std::vector<impl::port>>(
      impl::parse_ports(impl::fields::receivers(settings)) |
      boost::adaptors::filtered(impl::is_ws_port));
  const auto ws_receiver_ids = impl::make_ids(
      seed_id, nmos::types::receiver, ws_receiver_ports, receivers_count_total);
...
make_node_implementation_events_ws_message_handler(
  ...
  // the message handler will be used for all Events WebSocket connections, and
  // each connection may potentially have subscriptions to a number of sources,
  // for multiple receivers, so this example uses a handler adaptor that enables
  // simple processing of "state" messages (events) per receiver
          ...
          if (nmos::is_matching_event_type(
            ...
            slog::log<slog::severities::more_info>(gate, SLOG_FLF)
                ...
                << event_type.name << ")";
                         ...
                         nmos::event_types::wildcard(nmos::event_types::string),
                         event_type)) {
            slog::log<slog::severities::more_info>(gate, SLOG_FLF)
                << nmos::stash_category(impl::categories::node_implementation)
                << "Event received: "
...
make_node_implementation_connection_activation_handler(
  ...
  auto connection_events_activation_handler =
      nmos::make_connection_events_websocket_activation_handler(
          ...
          model.settings, gate);
  // this example uses this callback to update IS-12 Receiver-Monitor connection
  // status
...
make_node_implementation_map_validator() {
  // this example uses an 'empty' std::function because it does not need to do
  // any validation beyond what is expressed by the schemas and /caps endpoints
  return {};
}
...
make_node_implementation_channelmapping_activation_handler(
    slog::base_gate &gate) {
