...
CmdPassClient::CmdPassClient(std::string interface, std::string port)
    : pending_requests_(0) {
  ...
  std::shared_ptr<Channel> channel =
      grpc::CreateChannel(channel_config, grpc::InsecureChannelCredentials());
...
void CmdPassClient::FFmpegCmdExec(
  ...
  call->response_reader =
      stub_->PrepareAsyncFFmpegCmdExec(&call->context, req_obj, &cq_);
...
void CmdPassClient::AsyncCompleteRpc() {
  ...
  while (cq_.Next(&got_tag, &ok)) {
    ...
    if (ok) {
      if (call->status.ok()) {
        std::cout << "FFmpeg command executed successfully: "
                  << call->status.error_code() << std::endl;
      } else {
