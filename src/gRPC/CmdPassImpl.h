...
#define FFMPEG_INVALID_COMMAND_MSG                                             \
  std::string("Failed to execute ffmpeg command : No commands provided")

#define FFMPEG_APP_EXEC_FAIL_STATUS "-1"
#define FFMPEG_APP_EXEC_FAIL_MSG                                               \
  std::string("Failed to execute ffmpeg pipeline (popen) : ") +                \
      std::strerror(errno)
...
#define FFMPEG_EXEC_OK_MSG                                                     \
  std::string("FFmpeg command : ") + std::strerror(errno)
