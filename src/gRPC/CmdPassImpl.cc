...
static Config stringPairsToConfig(
    const std::vector<std::pair<std::string, std::string>> &pairs) {
  Config config;

  if (deserialize_config_json(config, pairs.front().second) != 0) {
    std::cout
        << "Error deserializing Config from json, trying previous solution"
        << std::endl;
  }
...
CmdPassImpl::CallData::CallData(CmdPass::AsyncService *service,
...
void CmdPassImpl::CallData::Proceed() {
  if (status_ == CREATE) {
    ...
    if (ffmpeg_generate_pipeline(recieved_config, pipeline_string) != 0) {
      ...
      std::cout << "Error generating pipeline"
                << std::endl; // TODO : need to return as response error code
      // return 1;
    }
    ...
    responder_.Finish(
        response_, Status(grpc::OK, FFMPEG_EXEC_OK_STATUS, FFMPEG_EXEC_OK_MSG),
        this);
