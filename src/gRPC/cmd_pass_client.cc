...
int main(int argc, char *argv[]) {
  ...
  if (argc != 5) {
    std::cout << "client sample app requires the following arguments: 1) "
                 "interface, 2) port, 3) source_ip, 4) destination_port"
              << std::endl;
    return 1;
  }
  ...
  std::vector<std::pair<std::string, std::string>> connection_info = {
      {"ip_addr", source_ip}, {"port", destination_port}};
