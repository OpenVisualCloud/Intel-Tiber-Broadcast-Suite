...
int ffmpeg_append_payload(Payload &p, std::string &pipeline_string) {
  ...
  case video:
    if (p.video.video_type == "rawvideo") {
      pipeline_string += " -video_size " + std::to_string(p.video.frame_width) +
                         "x" + std::to_string(p.video.frame_height);
      pipeline_string += " -pix_fmt " + p.video.pixel_format;
      pipeline_string += " -r " + std::to_string(p.video.frame_rate.numerator) +
                         "/" + std::to_string(p.video.frame_rate.denominator);
...
int ffmpeg_append_stream_convert(Payload &rx, Payload &tx,
  ...
  if (rx.type != payload_type::video) {
    std::cout << "Error: audio payload conversion not supported yet"
              << std::endl;
    return 1;
  }
  ...
  if (rx.video.frame_width != tx.video.frame_width ||
      rx.video.frame_height != tx.video.frame_height) {
    pipeline_string += " -vf scale=" + std::to_string(tx.video.frame_width) +
                       "x" + std::to_string(tx.video.frame_height);
  }
  if (rx.video.frame_rate.numerator != tx.video.frame_rate.numerator ||
      rx.video.frame_rate.denominator != tx.video.frame_rate.denominator) {
    pipeline_string += " -r " + std::to_string(tx.video.frame_rate.numerator) +
                       "/" + std::to_string(tx.video.frame_rate.denominator);
  }

  return 0;
}

int ffmpeg_append_st2110_transport(std::string &transport,
                                   std::string &pipeline_string) {
  if (transport == "st2110-20") {
    ...
    std::cout << "Error: transport " << transport << " not supported yet"
              << std::endl;
    return 1;
  }
  return 0;
...
int ffmpeg_append_mcm_transport(Payload &p, std::string &pipeline_string) {
  ...
  case video:
    ...
    break;
  case audio:
    if (p.audio.format == "s16be" || p.audio.format == "s16le" ||
        p.audio.format == "u16be" || p.audio.format == "u16le") {
      pipeline_string += " -f mcm_audio_pcm16";
      break;
    } else if (p.audio.format == "s24be" || p.audio.format == "s24le" ||
               p.audio.format == "u24be" || p.audio.format == "u24le") {
      pipeline_string += " -f mcm_audio_pcm24";
      break;
    } else {
      std::cout << "Error: audio format " << p.audio.format
...
int ffmpeg_append_stream_type(Stream &st, bool is_rx, int idx,
                              std::string &pipeline_string) {
  ...
  case file: {
    ...
    if (!is_rx) {
      if (st.payload.video.video_type == "rawvideo") {
        pipeline_string += "-video_size " +
                           std::to_string(st.payload.video.frame_width) + "x" +
                           std::to_string(st.payload.video.frame_height);
        pipeline_string += " -pix_fmt " + st.payload.video.pixel_format;
        pipeline_string +=
            " -r " + std::to_string(st.payload.video.frame_rate.numerator) +
            "/" + std::to_string(st.payload.video.frame_rate.denominator);
        pipeline_string += " -f rawvideo ";
  ...
  case st2110:
    ...
    pipeline_string +=
        " -payload_type " + std::to_string(s.st2110.payload_type);
    if (is_rx) {
    ...
    if (ffmpeg_append_st2110_transport(s.st2110.transport, pipeline_string) !=
        0) {
  ...
  case mcm:
    ...
    if (s.mcm.transport == "st2110-20") {
      pipeline_string +=
          " -transport_pixel_format " + s.mcm.transport_pixel_format;
    }
...
int ffmpeg_combine_rx_tx(Stream &rx, Stream &tx, int idx,
                         std::string &pipeline_string) {
  ...
  if (ffmpeg_append_stream_type(rx, true /*is_rx*/, idx, pipeline_string) !=
      0) {
    pipeline_string.clear();
  ...
  if (ffmpeg_append_stream_type(tx, false /*is_rx*/, idx, pipeline_string) !=
      0) {
    pipeline_string.clear();
...
int ffmpeg_append_multiviewer_input(Stream &s, int idx,
                                    std::string &pipeline_string) {
  if (s.payload.type != payload_type::video) {
    std::cout << "Error: multiviewer requires video payload all receivers"
              << std::endl;
    return 1;
  }
...
int ffmpeg_append_multiviewer_process(std::vector<Stream> &receivers,
                                      Video &output_video, uint32_t columns,
                                      uint32_t intel_gpu,
                                      std::string &pipeline_string) {
  ...
  for (int i = 0; i < receivers.size(); i++) {
    ...
    pipeline_string += std::to_string(single_screen_width) + ":" +
                       std::to_string(single_screen_height) + "[out" +
                       std::to_string(i) + "];";
  }
  for (int i = 0; i < receivers.size(); i++) {
  ...
  if (intel_gpu) {
    pipeline_string +=
        ",hwdownload,format=y210le,format=" + output_video.pixel_format + "\"";
  } else {
...
int ffmpeg_append_split_process(std::vector<Stream> &senders,
                                uint32_t intel_gpu,
                                std::string &pipeline_string) {
  pipeline_string +=
      " -filter_complex \"split=" + std::to_string(senders.size());
  ...
  for (int i = 0; i < senders.size(); i++) {
    if (intel_gpu) {
      pipeline_string +=
          "[in" + std::to_string(i) + "]hwupload,scale_qsv=" +
          std::to_string(senders[i].payload.video.frame_width) + ":" +
          std::to_string(senders[i].payload.video.frame_height) +
          ",hwdownload,format=" + senders[i].payload.video.pixel_format +
          "[out_tmp" + std::to_string(i) + "];";
    } else {
      pipeline_string +=
          "[in" + std::to_string(i) +
          "]scale=" + std::to_string(senders[i].payload.video.frame_width) +
          ":" + std::to_string(senders[i].payload.video.frame_height) + "[out" +
          std::to_string(i) + "];";
    }
  }
  pipeline_string += "\"";
...
int ffmpeg_append_rx_tx(Config &config, std::string &pipeline_string) {
  if ((config.receivers.size() == 0) ||
      (config.receivers.size() != config.senders.size())) {
    std::cout
        << "Error: function " << config.function
        << " requires equal number of receivers and senders, greater than 0"
        << std::endl;
    return 1;
  } else {
    for (int i = 0; i < config.receivers.size(); i++) {
      if (ffmpeg_combine_rx_tx(config.receivers[i], config.senders[i], i,
                               pipeline_string) != 0) {
        pipeline_string.clear();
...
int ffmpeg_append_multiviewer(Config &config, std::string &pipeline_string) {
  ...
  if (config.receivers.size() < 2) {
    std::cout << "Error: multiviewer requires at least 2 receivers"
              << std::endl;
    return 1;
  }
  if (config.multiviewer_columns < 1 ||
      config.multiviewer_columns > 50) { // Soft limit of 50 columns
    std::cout << "Error: multiviewer requires number of columns to be greater "
                 "than 0 and less than 50, got "
              << std::to_string(config.multiviewer_columns) << std::endl;
    return 1;
  }

  for (int i = 0; i < config.receivers.size(); i++) {
    if (ffmpeg_append_multiviewer_input(config.receivers[i], i,
                                        pipeline_string) != 0) {
      pipeline_string.clear();
  ...
  if (ffmpeg_append_multiviewer_process(
          config.receivers, config.senders[0].payload.video,
          config.multiviewer_columns, config.gpu_hw_acceleration == "intel",
          pipeline_string) != 0) {
    pipeline_string.clear();
    std::cout << "Error appending multiviewer process" << std::endl;
    return 1;
  }
  if (ffmpeg_append_stream_type(config.senders[0], false /*is_rx*/, 0,
                                pipeline_string) != 0) {
    pipeline_string.clear();
...
int ffmpeg_append_recorder(Config &config, std::string &pipeline_string) {
  ...
  if (ffmpeg_append_payload(config.receivers[0].payload, pipeline_string) !=
      0) {
    pipeline_string.clear();
    std::cout << "Error appending recorder rx payload" << std::endl;
    return 1;
  }
  if (ffmpeg_append_stream_type(config.receivers[0], true /*is_rx*/, 0,
                                pipeline_string) != 0) {
    pipeline_string.clear();
    std::cout << "Error appending recorder rx stream" << std::endl;
    return 1;
  }
  if (ffmpeg_append_split_process(config.senders,
...
int ffmpeg_append_upscale(Config &config, std::string &pipeline_string) {
  if (config.receivers.size() != 1 || config.senders.size() != 1) {
    std::cout << "Error: upscale is requires exactly 1 receiver and sender"
              << std::endl;
    return 1;
  }
  if (config.receivers[0].payload.type != payload_type::video ||
      config.senders[0].payload.type != payload_type::video) {
    std::cout << "Error: upscale requires video payload" << std::endl;
    return 1;
  }
  if ((2 * config.receivers[0].payload.video.frame_width) !=
          config.senders[0].payload.video.frame_width ||
      (2 * config.receivers[0].payload.video.frame_height) !=
          config.senders[0].payload.video.frame_height) {
    std::cout << "Error: upscale uses the raisir library that upscales width "
                 "and height by a power of 2"
              << std::endl;
    return 1;
  }
  ...
  pipeline_string += " -vf "
                     "\"format=yuv420p,hwupload,raisr_opencl,hwdownload,format="
                     "yuv420p,format=yuv422p10le\"";
  if (ffmpeg_append_stream_type(config.senders[0], false /*is_rx*/, 0,
                                pipeline_string) != 0) {
    pipeline_string.clear();
...
int ffmpeg_generate_pipeline(Config &config, std::string &pipeline_string) {
  ...
  if (config.gpu_hw_acceleration == "none") {
    ...
    if (config.gpu_hw_acceleration_device.empty()) {
      std::cout << "Error: Intel GPU acceleration requires a device path"
                << std::endl;
      return 1;
    }
    pipeline_string += " -y -qsv_device " + config.gpu_hw_acceleration_device +
                       " -hwaccel qsv";
  } else if (config.gpu_hw_acceleration == "nvidia") {
    pipeline_string += " -y -hwaccel cuda -hwaccel_output_format cuda";
  } else {
    std::cout << "Unsupported GPU acceleration " << config.gpu_hw_acceleration
              << std::endl;
    return 1;
  }
  ...
  if (config.function == "tx" || config.function == "rx") {
    ...
    std::cout << "Error: function " << config.function << " not supported yet"
              << std::endl;
    return 1;
  }
